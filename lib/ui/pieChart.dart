  ...
  void paint(Canvas canvas, Size size) {
    Paint paint =
        Paint() // 화면에 그릴 때 쓸 Paint를 정의합니다.
          ...
          ..strokeCap = StrokeCap
              .square; // stroke의 스타일을 정합니다. round를 고르면 stroke의 끝이 둥글게 됩니다.
    ...
    double radius = min(
        ...
        size.height / 2 -
            paint.strokeWidth /
                2); // 원의 반지름을 구함. 선의 굵기에 영향을 받지 않게 보정함.
    Offset center = Offset(
        size.width / 2,
        size.height /
            2); // 원이 위젯의 가운데에 그려지게 좌표를 정함.
    ...
    paint
      ..color = Colors
          .deepPurpleAccent; // 호를 그릴 때는 색을 바꿔줌.
    canvas.drawArc(
        Rect.fromCircle(
            center: center,
            radius: radius),
  ...
  void drawText(Canvas canvas,
    ...
    TextSpan sp = TextSpan(
        style: TextStyle(
            ...
            color: Colors.black),
        text:
            text); // TextSpan은 Text위젯과 거의 동일하다.
